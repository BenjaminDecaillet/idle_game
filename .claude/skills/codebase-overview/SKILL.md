...
- `engine.ts` — `tick()` (per-country: salaries+debt incl. event salary boosts, per-project work with trait/site-specialty multipliers, XP with grade caps, timed actions at company AND country level, soft-capped completions + vault accrual), `createInitialState()`/`createCountry()`, all player actions (errors return `error.*` i18n ids), country accessors, builder pool, shop, vault (`vaultCap`/`openVault`), pets (`buyPet`/`setCompanyPet`), traits (`rollTraits`, `traitOutputMult`/`traitSalaryMult`/`traitXpMult`, `workerSalary`), `SAVE_VERSION` (v10), company-tier cost scaling (`companyCostScale`/`companySalaryScale`, balance.md Phase S)
- `daily.ts` — daily contracts (day-seeded board, delta baselines; day number passed in from main.ts)
- `events.ts` — random opportunity offers (`rollEventOffer`/`acceptEventOffer`; UI schedules, engine resolves)
...
`ui.ts` (5 tabs — Map, Office (drill-down: companies → building → floor/staff room, hire bottom-sheet), Shop (funding rounds + vault), VsCoin (daily contracts + missions + SKUs), Stats; 2 Hz innerHTML re-render + 60 fps HUD `textContent` writes, `buildSkeleton()` owns HUD ids and rebuilds on language change; badge chips: vault, builders, VsCoin/missions dot; event/story modals share `modal-zone`), `officeScene.ts` and `cityMap.ts` (large SVG scenes; `cityMapSvg(themeId, sites, countryId)` with per-country `COUNTRY_THEMES`), `persona.ts` (deterministic per-worker cartoon art), `portraits.ts`/`portraitArt.ts`/`gabrielPortrait.ts` (painted portraits), `itemArt.ts`, `icons.ts`, `fx.ts` (canvas particles, SFX/chimes, chiptune loop — see the audio-chiptune skill), `gabriel.ts` (dialog panel), `coachPlacement.ts` (tutorial popup anchoring).
