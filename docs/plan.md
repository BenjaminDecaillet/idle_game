...
3. **What to check first:** on the Office tab, tap Hire and take a candidate, then buy a Basic Desk — the Landing Page project starts filling and auto-repeats with confetti on payout. Close the tab, come back later → "Welcome back" modal with offline earnings. Music is off by default (Stats → Settings).
...
- [x] **Idle-genre state-of-the-art survey** — OSS repos (licences checked), design theory and QoL conventions distilled into [idle-game-state-of-the-art.md](idle-game-state-of-the-art.md); backlog items #23-#36 in [improvements.md](improvements.md); immediate picks: save export/import, bulk buy, itemized welcome-back report
